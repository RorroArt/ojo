...
def gq_attention(
    ...
    xq = jnp.einsum('ld,dk->lk', x, q_proj)
    xk = jnp.einsum('ld,dk->lk', x, k_proj)
    xv = jnp.einsum('ld,dk->lk', x, v_proj)

    xq = rearrange(xq, 'l (h k) -> l h k', h=NH, k=HD)
    xk = rearrange(xk, 'l (h k) -> l h k', h=NKVH, k=HD)
    xv = rearrange(xv, 'l (h k) -> l h k', h=NKVH, k=HD)

    new_k_cache, new_v_cache = update_kv_cache(k_cache, v_cache, xk, xv, starting_pos)

    kr, vr = extract_relevant_kv_cache(new_k_cache, new_v_cache, L, starting_pos)

    if G > 1:
        kr = jnp.repeat(kr, G, axis=1)
        vr = jnp.repeat(vr, G, axis=1)
    ...
    scores = jnp.einsum('hik,hjk->hij', xq, kr) * scale
    ...
    out = jnp.einsum('hij,hjk->hik', scores, vr)
    out = rearrange(out, 'h l k -> l (h k)')
    out = jnp.einsum('lk,kd->ld', out, o_proj)
